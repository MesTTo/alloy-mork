...
| `fac27_iter_token` | UNSAT×2/SAT | the u64 trie iteration token is sound: with the token as a node-local cursor (one more than the last byte visited, next byte recomputed from the node's mask as the minimum set byte ≥ token), the walk visits exactly the mask's bytes, each once, in ascending order, and resuming from byte k covers exactly the bytes strictly greater than k — the contract the old mask-caching 128-bit token provided, now independent of token width |
